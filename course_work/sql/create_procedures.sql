...
CREATE OR REPLACE FUNCTION  create_adventure(
    ...
    BEGIN
        SELECT COUNT(DISTINCT planets_planet.universe_id) INTO universes_count FROM planets_planet WHERE planets_planet.id = ANY(planets_ids);
        ...
        FOREACH planet_id IN ARRAY planets_ids LOOP
                INSERT INTO adventures_adventureplanet (adventure_id, planet_id, is_visited) VALUES (adv_id, planet_id, false);
            END LOOP;
...
CREATE OR REPLACE PROCEDURE fill_battle_members(
    ...
    BEGIN
        ...
        INSERT INTO battles_battlemember (battle_id, battler_id, is_opponent) SELECT _battle_id, battler_id, false FROM teammates_creatures;
...
CREATE OR REPLACE PROCEDURE fill_battler_effects(_battle_id INTEGER) AS $$
    ...
    BEGIN
        ...
        CREATE TEMP TABLE creatures_opps AS (
            SELECT battler_id, creature_id, is_opponent FROM battles_battlemember JOIN battles_battler ON battles_battlemember.battler_id = battles_battler.id
        );
        ...
        SELECT _battle_id, battler_id, effect_id FROM (
            SELECT * FROM affects_planeteffectrule
        ...
        INSERT INTO battles_battlereffect (battle_id, battler_id, effect_id)
        SELECT _battle_id, battler_id, effect_id FROM (
            SELECT al_to.battler_id, effect_id FROM affects_creatureeffectrule
        ...
        INSERT INTO battles_battlereffect (battle_id, battler_id, effect_id)
        SELECT _battle_id, battler_id, effect_id FROM (
            SELECT op_to.battler_id, effect_id FROM affects_creatureeffectrule
                INNER JOIN allies al_from ON al_from.creature_id = creature_from_id
...
CREATE OR REPLACE PROCEDURE make_battle_report(_battle_id INTEGER) AS $$
    ...
    BEGIN
        ...
        CREATE TEMP TABLE battlers_with_creatures_and_weapons AS (
            SELECT
                creatures_creature.id as creature_id,
        ...
        CREATE TEMP TABLE battler_applied_effects AS (
            SELECT is_opponent, power_affect FROM battles_battlemember
        ...
        INSERT INTO battles_battlereport (battle_id, allies_power, opponents_power) VALUES (_battle_id, _allies_power, _opponents_power);
...
CREATE OR REPLACE FUNCTION adventure_step(adv_id INTEGER) RETURNS BOOL AS $$
    ...
    BEGIN
        ...
        WHERE adventures_adventureplanet.adventure_id = adv_id AND is_visited = False ORDER BY random() LIMIT 1;
        ...
        IF(_adventure_started_at IS NULL) THEN UPDATE adventures_adventure SET started_at=current_timestamp WHERE id=adv_id; END IF;
        UPDATE adventures_adventureplanet SET is_visited=True WHERE id = adventure_planet_id;

        INSERT INTO battles_battle (adventure_id, planet_id, created_at) VALUES
        ...
        SELECT allies_power, opponents_power INTO _allies_power, _opponents_power FROM battles_battlereport WHERE battle_id=_battle_id;
